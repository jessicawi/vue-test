<template>
    <div class="">
        <div class="row">
            <div class="col-12">
                <b-card no-body class="mb-1" v-for="tempobj_Year of arrobj_Levels" :key="tempobj_Year.id">
                    ...
                    <b-collapse :id="tempobj_Year.Str_SortBy" visible role="tabpanel">
                        <b-card-body>
                            <b-card no-body class="mb-1" v-for="tempobj_Level of tempobj_Year.ArrObj_Items"
                                    :key="tempobj_Level.id">
                                <b-card-header header-tag="header" class="p-1" role="tab">
                                    <div class="row">
                                        <div class="col-10">
                                            <div class="btn btn-warning btn-block">
                                                <div v-b-toggle="tempobj_Level.Str_SortBy">
                                                    {{tempobj_Level.Str_Display}}
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-2">
                                ...
                                <b-collapse :id="tempobj_Level.Str_SortBy" accordion="level" role="tabpanel">
                                    <b-card-body>
                                        <b-card no-body class="mb-1" v-for="tempobj_Class of tempobj_Level.ArrObj_Items"
                                                :key="tempobj_Class.id">
                                            <b-card-header header-tag="header" class="p-1" role="tab">
                                                <div class="row">
                                                    <div class="col-10">
                                                        <div class="btn btn-secondary btn-block">
                                                            <div v-b-toggle="(tempobj_Level.Str_SortBy + tempobj_Class.Str_SortBy).replace(' ', '_')">
                                                                {{tempobj_Class.Str_Display}}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-2">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initTags() {
                this.arrobj_SelectedStudents = this.parent_Post.arrobj_SelectedStudents.map(x => x.PostTagUserID);
            ...
            sortByClass() {
                this.getStudents().then((result) => {
                    ...
                    for (let i = 0; i < sortedList.length; i++) {
                        let tempLevel = this.groupBy(sortedList[i].ArrObj_Items, "PK_Class_ID", "CLS_ClassName,CLS_Batch");

                        sortedList[i].ArrObj_Items = tempLevel;
                    }


                    this.arrobj_Classes = sortedList;
                }).finally(() => {
                    this.hideLoading();
                });
            },
            ...
            verifyCheckall(obj) {
                let siblingsCheckbox = $(obj).closest(".collapse").find("input[type='checkbox']");
                let checkallCheckbox = $(obj).closest(".collapse").prev().find("input[type='checkbox']");
                let checkall = true;
                ...
                checkallCheckbox.prop("checked", checkall);
            },
        },
        mounted() {
            const self = this;
            ...
            $(document).on("change", ".cb_CheckAll", function () {
                let check = $(this).prop("checked");
                let checkboxes = $(this).closest("header").parent().find("input[type='checkbox']");
            ...
            $(document).on("change", "input[type='checkbox']", function () {
                ...
                // self.isCheck($(this).attr("id"));
                self.verifyCheckall(this)
                self.$emit("tags", self.arrobj_SelectedStudents);
            });
        },
        updated() {
            const self = this;
